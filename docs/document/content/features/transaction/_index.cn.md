...
ShardingSphere BASE 事务使用 Seata AT 模式。分支事务在第一阶段提交本地事务后释放数据库本地锁，但全局写锁会保持到全局事务结束。
因此，BASE 不会像 XA 一样在整个事务期间持有数据库本地锁，但热点数据仍可能产生全局锁竞争。
通过放宽强一致性要求获得的吞吐量提升取决于具体负载。
...
| 一致性  | 不支持      | 支持        | 最终一致            |
| 隔离性  | 不支持      | 支持        | Seata AT 全局锁      |
| 并发性能 | 无影响      | 严重衰退      | 略微衰退            |
| 适合场景 | 业务方处理不一致 | 短事务 & 低并发 | 最终一致 & 低热点竞争    |

### 隐式分布式事务

当连接处于自动提交模式、事务规则的默认类型为 XA 或 BASE、当前没有活动的分布式事务，且一条写 DML 路由到多个执行单元时，
...
BASE 事务在分布式环境下提供最终一致性。第一阶段完成后会释放数据库本地锁，但全局写锁会保持到全局事务结束。
它适用于允许短暂数据不一致，并且热点写入竞争可控的业务场景。
