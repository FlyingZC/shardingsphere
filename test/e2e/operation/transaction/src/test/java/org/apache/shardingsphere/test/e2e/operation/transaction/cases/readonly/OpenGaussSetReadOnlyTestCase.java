...
/**
 * openGauss set read only transaction integration test.
 */
@TransactionTestCase(dbTypes = TransactionTestConstants.OPENGAUSS)
public final class OpenGaussSetReadOnlyTestCase extends SetReadOnlyTestCase {
    
    public OpenGaussSetReadOnlyTestCase(final TransactionTestCaseParameter testCaseParam) {
        super(testCaseParam);
    }
    ...
    private void assertSetReadOnly() throws SQLException {
        ...
        try (Connection connection = getDataSource().getConnection()) {
            ...
            assertReadOnlyUpdate(connection);
        }
        try (Connection connection = getDataSource().getConnection()) {
    ...
    private void assertReadOnlyUpdate(final Connection connection) {
